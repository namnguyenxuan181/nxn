...
from scrapers.vnstock_scraper import VnstockScraper, _fetch_ohlc, _to_vnd
...
def test_fetch_ohlc_returns_stock_price():
    with patch("scrapers.vnstock_scraper.requests.get", return_value=_ohlc_mock()):
...
def test_fetch_ohlc_returns_none_when_no_data():
    ...
    with patch("scrapers.vnstock_scraper.requests.get", return_value=_ohlc_mock(empty)):
        result = _fetch_ohlc("HPG", "2026-06-18")
    assert result is None


def test_fetch_ohlc_returns_none_on_network_error():
    with patch("scrapers.vnstock_scraper.requests.get", side_effect=Exception("timeout")):
        result = _fetch_ohlc("HPG", "2026-06-18")
    assert result is None
...
def test_get_symbols_parses_csv():
    ...
    with patch("scrapers.vnstock_scraper.requests.get", return_value=_listing_mock()):
...
def test_scrape_returns_sorted_results():
    ...
    with patch("scrapers.vnstock_scraper.requests.get", side_effect=_side_effect_factory()):
...
def test_scrape_filters_none_results():
    ...
    scraper = VnstockScraper(target_date="2026-06-18", max_workers=2)
    with patch(
        "scrapers.vnstock_scraper.requests.get",
...
def test_scrape_raises_when_listing_fetch_fails():
    scraper = VnstockScraper(target_date="2026-06-18")
    with patch("scrapers.vnstock_scraper.requests.get", side_effect=Exception("network error")):

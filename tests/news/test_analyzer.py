...
def test_analyze_article_returns_summary_and_sentiment():
    with patch("services.news.analyzer.anthropic.Anthropic", return_value=_mock_client(
...
def test_analyze_article_returns_neutral_on_error():
    with patch("services.news.analyzer.anthropic.Anthropic", side_effect=Exception("API error")):
        with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"}):
...
def test_analyze_article_handles_invalid_json():
    ...
    client = MagicMock()
    client.messages.create.return_value = message
    with patch("services.news.analyzer.anthropic.Anthropic", return_value=client):
        with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"}):
            summary, sentiment = analyze_article("Title", "Desc")
    assert summary == ""
    assert sentiment == "neutral"

...
from services.stock.scrapers.intraday import is_market_open, fetch_intraday_prices
...
def test_is_market_open_weekday_morning():
    with patch("services.stock.scrapers.intraday.datetime") as mock_dt:
...
def test_fetch_intraday_prices_returns_vnd():
    ...
    with patch("services.stock.scrapers.intraday.requests.get", return_value=mock_resp):
...
def test_fetch_intraday_prices_skips_on_exception():
    with patch("services.stock.scrapers.intraday.requests.get", side_effect=Exception("timeout")):
        result = fetch_intraday_prices(["TCB"])
    assert result == {}
...
def test_fetch_intraday_prices_multiple_symbols():
    ...
    with patch("services.stock.scrapers.intraday.requests.get", side_effect=_side_effect):
